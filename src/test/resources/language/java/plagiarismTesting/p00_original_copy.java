...
/*
 * Program muze obcas pri debugu na konci vyhodit:
 * ERROR: JDWP Unable to get JNI 1.2 environment, jvm->GetEnv() return code = -2
 * JDWP exit error AGENT_ERROR_NO_JNI_ENV(183):  [../../../src/share/back/util.c:838]
 * 
 * Zjisteno az po doprogrovani. Nejedna se o chybu na me strane, ale nio+java chyba.
 * Oracle tuto chybu nedokazal opravit behem 9 let. http://bugs.java.com/view_bug.do?bug_id=6476706
 * */
...
public class Robot {
    ...
    //Active connection either upload or download 
    static public Connection con;
    
    //Volatile becouse of multiple thread primitive (== and =) accessing.
	static public volatile long T=0;
    static public volatile Integer connection=null; //ID of connection, using object not primitive becouse of small chance that ID will be 0
	...
	public static void main(String... args) throws IOException
	{
		...
		/* ZACATEK PRIPOJOVANI */
	    localport = new InetSocketAddress(0); //Vyber volneho portu

		channel = DatagramChannel.open();
	    socket=channel.socket();
	    socket.setSoTimeout(90000); //Neni v zadani. Pro jistotu socket zavre po 90s
	    socket.bind(localport);
	    remote=new InetSocketAddress(addr,port);
	    channel.connect(remote);

	    /* ZACATEK POSILANI DAT A NASTAVENI DRUHU DAT */
	    ...
	    /* VYTVORENI ZAPISOVACIHO THREDU. POUZIVAN HLAVNE PRO UPLOAD.*/
...
abstract class Connection
{
	...
	/* Druh pripojeni at se zbytecne nedela instanceof pro check. */
	public abstract byte getType();
	
	/* Pro bezpecne uzavreni otevrenych souboru */
	public abstract void endCon();
	
	/* Volano z writeThreadu */
	public abstract void sendPacket() throws IOException;
	/* Volano z readThreadu pri prichodu ZVALIDOVANEHO packetu. */
...
class UploadConnection extends Connection
{
	...
	/** Vytvori novy packet POKUD zbyvaji v firmwaru data. */
	...
	/** Doplni okenko a podle C posle data. C=false: posle vsechny data.  C=true: posle jen nova data v okenku.*/
...
/** Packet uklada data do ByteBufferu. Slozitejsi pro vyznani, ale rychlejsi. */
class Packet {
	ByteBuffer data;
	int checkSeq;
	/** Zvaliduje packet a pripadne rovnou odesle error.*/
...
/**Paralerni thread zajistujici poslani novych dat. */
